...
export const fetchGamesBySection = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (!force && !background) {
        ...
        if (cachedGames && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            console.log(`[Redux] Using cached games for section "${uiSection}" (age: ${Math.round(cacheAge / 1000)}s)`);
            
            // Trigger background refresh if cache is 80% expired (4 minutes)
            if (cacheAge > cacheTTL * 0.8) {
              console.log(`[Redux] Cache is 80% expired, triggering background refresh for "${uiSection}"`);
              // Dispatch background refresh (don't await)
              setTimeout(() => {
                const { store } = require('@/lib/redux/store');
                store.dispatch(fetchGamesBySection({
                  uiSection,
                  ageGroup,
                  gender,
                  page,
          ...
          console.log(`[Redux] Cache is stale for "${uiSection}" (age: ${Math.round(cacheAge / 1000)}s), refreshing in background`);
      ...
      console.log(`[Redux] ${background ? '[BACKGROUND] ' : ''}Fetching games by section with params:`, {
        uiSection,
        ageGroup,
        gender,
        page,
    ...
    try {
      ...
      console.log("[Redux] Returning game data:", {
        ...
        gameIdMatch: gameData
          ? (gameData.id || gameData._id || gameData.gameId) === gameId ||
            gameData.gameId === gameId ||
            gameData.gameDetails?.id === gameId
          : false,
      });
  ...
  reducers: {
    ...
    clearSpecificSection: (state, action) => {
      ...
      if (state.gamesBySection[sectionName]) {
        ...
        console.log(`[Redux] Cleared data and timestamp for section: ${sectionName}`);
      }
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchGamesBySection.pending, (state, action) => {
        ...
        state.error = null;
        console.log(`[Redux] ${isBackground ? '[BACKGROUND - NO LOADING STATE] ' : ''}Loading games for section: ${uiSection}`);
      })
      .addCase(fetchGamesBySection.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          state.gamesBySectionTimestamp[uiSection] = action.payload.timestamp || Date.now();
          state.availableUiSections = action.payload.uiSections || state.availableUiSections;
          
          console.log(
            `[Redux] ${isBackground ? '[BACKGROUND] ' : ''}Games for section "${uiSection}" ${fromCache ? 'served from cache' : 'loaded successfully'}:`,
            action.payload.games.length,
            "games",
            fromCache ? `(cache age: ${Math.round(action.payload.cacheAge / 1000)}s)` : ''
          );
        } else {
