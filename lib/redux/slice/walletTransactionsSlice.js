  ...
  async (
    ...
    try {
      ...
      if (!force && !background) {
        ...
        const cacheTimestamp = state.walletTransactions.walletScreenCacheTimestamp;
        ...
        if (cachedWalletScreen && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            console.log(`[Redux] Using cached wallet screen (age: ${Math.round(cacheAge / 1000)}s)`);

            // Trigger background refresh if cache is 80% expired (4 minutes)
            if (cacheAge > cacheTTL * 0.8) {
              console.log(`[Redux] Wallet screen cache is 80% expired, triggering background refresh`);
              setTimeout(() => {
                const { store } = require('@/lib/redux/store');
                store.dispatch(fetchWalletScreen({
                  token,
                  background: true
                }));
              }, 0);
          ...
          console.log(`[Redux] Wallet screen cache is stale (age: ${Math.round(cacheAge / 1000)}s), refreshing in background`);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchWalletScreen.pending, (state, action) => {
        ...
        state.error = null;
        console.log(`[Redux] ${isBackground ? '[BACKGROUND - NO LOADING STATE] ' : ''}Loading wallet screen`);
      })
      .addCase(fetchWalletScreen.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          const { fromCache: _, cacheAge: __, stale: ___, timestamp: ____, background: _____, ...walletScreenData } = action.payload;
          state.walletScreen = walletScreenData;
          state.walletScreenCacheTimestamp = Date.now();
          console.log(`[Redux] ${isBackground ? '[BACKGROUND] ' : ''}Updated wallet screen with fresh data`);
        } else {
          console.log(`[Redux] Using cached wallet screen (cache age: ${Math.round((action.payload.cacheAge || 0) / 1000)}s)`);
        }
